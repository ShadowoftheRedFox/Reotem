...
NotifRouter.post('/num/:id', async (req, res, next) => {
    try {
        logger(`[NOTIFICATIONS] Getting user ${req.params.id} amount of notifications...`);
...
NotifRouter.post('/:id', async (req, res, next) => {
    try {
        logger(`[NOTIFICATIONS] Getting user ${req.params.id} notifications...`);
...
NotifRouter.put('/read', async (req, res, next) => {
    try {
        logger(`[NOTIFICATIONS] Setting some notifications from user ${req.body.session} to read...`);
...
NotifRouter.put('/unread', async (req, res, next) => {
    try {
        logger(`[NOTIFICATIONS] Setting some notifications from user ${req.body.session} to unread...`);
...
NotifRouter.delete('/', async (req, res, next) => {
    try {
        logger(`[NOTIFICATIONS] Deleting some notifications from user ${req.body.session}...`);
